...
import {TreeNode,SharedModule,PrimeTemplate} from 'primeng/primeng';
    ...
    filterNodes(filter, tree) {
       ...
       let filterFn;
        ...
        if (typeof filter ==='string') {
              filterFn = (tree) => tree.label.toLowerCase().indexOf(filter.toLowerCase()) !== -1;
        }
        ...
        const ids = {};
        tree.forEach(
          (node) => this._filterNode(ids, node, filterFn, null)
      );
...
export class MytreemoduleModule { }
